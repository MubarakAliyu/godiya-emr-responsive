...
export function AppointmentsPage() {
  const { appointments, patients, settings } = useEMRStore();
  ...
  const handlePrint = (appointment: Appointment) => {
    printPOSSlip(settings, {
      title: 'Appointment & Payment Receipt',
      sections: [
        {
          title: 'Appointment Info',
          items: [
            { label: 'Slip Number', value: appointment.id },
        ...
        },
        {
          title: 'Payment Info',
          items: [
            { label: 'Receipt No', value: `RE-${Math.floor(1000 + Math.random() * 9000)}` },
            { label: 'Service', value: `Consultation (${appointment.appointmentType})` },
            { label: 'Amount', value: `${settings.general?.currencySymbol || '₦'}5,000.00` },
            { label: 'Status', value: appointment.isPaid ? 'PAID' : 'UNPAID' },
          ],
        }
      ],
      footer: 'Please bring this slip to the consultation room. Thank you for choosing Godiya Hospital.'
    });
    toast.success('Printing merged receipt...');
  };
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-sm font-medium">
                      <div className="flex flex-col">
                        ...
                        {appointment.isSubfile && (
                          <span className="text-[10px] text-muted-foreground bg-muted w-fit px-1 rounded">Family Member</span>
                        )}
                      </div>
                    </td>
